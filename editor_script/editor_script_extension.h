...
/// EditorScript 的便捷基类：统一管理生命周期并封装常用编辑器辅助方法。[br][br]
...
public:
	...
	/// 获取当前主循环的 SceneTree。[br][br]
	/// [return] 当前主循环的 SceneTree。
	SceneTree *get_tree();
	/// 创建场景树计时器。[br][br]
	/// [param p_time_sec] 计时秒数[br]
	/// [param p_process_always] 是否在时间缩放时仍计时[br]
	/// [param p_process_in_physics] 是否在物理帧更新[br]
	...
	/// 创建一个以编辑器为主控件为父级的窗口。[br][br]
	/// [return] 新建的 Window，关闭时自动释放。
	Window *create_window();
	/// 创建一个以编辑器为主控件为父级的确认框。[br][br]
	/// [param p_dialog_text] 对话框文本[br]
	/// [param p_title] 窗口标题[br]
