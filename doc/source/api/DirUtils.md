...
**p_extensions：** （可选）参与计算的文件扩展名列表，如 ["png", "jpg"]。空列表表示所有文件，大小写不敏感。

**返回：** MD5 哈希字符串，若目录无法打开则返回空字符串。
...
**p_should_delete：** 判断是否删除的回调，签名为 func(file_path: String, file_name: String) -> bool
...
**返回：** 目录信息字典，含键：name(String)、path(String)、directories(Array[Dictionary](https://docs.godotengine.org/en/stable/classes/class_dictionary.html))、files(Array[String](https://docs.godotengine.org/en/stable/classes/class_string.html))；目录无效时返回空字典。
...
**p_file_handler：** 处理文件的回调，签名为 func(file_path: String, file_name: String) -> bool，返回 false 终止遍历

**p_dir_handler：** 处理目录的回调（可选），签名为 func(dir_path: String, dir_name: String) -> bool，返回 false 终止遍历

**p_recursive：** 是否递归子目录（默认为 true）

**返回：** 是否完整执行完毕（未被回调终止），true 表示完整遍历。
