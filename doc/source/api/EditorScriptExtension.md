...
**p_ok_button_text：** 确认按钮文本

**返回：** 新建的 AcceptDialog，关闭时自动释放。
...
**返回：** 新建的 ConfirmationDialog，关闭时自动释放。
...
**p_process_always：** 是否在时间缩放时仍计时

**p_process_in_physics：** 是否在物理帧更新

**p_ignore_time_scale：** 是否忽略时间缩放
