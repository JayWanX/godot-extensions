...
/// 文件系统工具类：提供文件类型判定等纯静态方法，不可实例化。[br][br]
...
public:
	/// 判断文件是否可视为文本文件。[br][br]
	/// 先按扩展名/文件名做零 IO 快路径判定，未命中再读取文件头查 NUL 字节兜底。[br]
	/// [param p_file] 文件路径，可为系统绝对路径或 res:// 路径。[br]
	/// [param p_max_read_bytes] 读取文件头的最大字节数。[br]
