...
public:
	/// 计算目录哈希（仅当前层文件，不含子目录）。
	/// [param p_path] 目录路径。
	/// [param p_extensions] （可选）参与计算的文件扩展名列表，如 ["png", "jpg"]。空列表表示所有文件，大小写不敏感。
	...
	/// [param p_path] 起始目录路径
	/// [param p_file_handler] 处理文件的回调，签名为 func(file_path: String, file_name: String) -> bool，返回 false 终止遍历
	/// [param p_dir_handler] 处理目录的回调（可选），签名为 func(dir_path: String, dir_name: String) -> bool，返回 false 终止遍历
	/// [param p_recursive] 是否递归子目录（默认为 true）
	...
	/// 根据条件删除文件，返回实际删除数量。
	/// [param p_root_path] 起始目录路径
	/// [param p_should_delete] 判断是否删除的回调，签名为 func(file_path: String, file_name: String) -> bool
	/// [return] 实际删除的文件数量。
