...
public:
	/// 计算目录哈希（仅当前层文件，不含子目录）。[br][br]
	/// [param p_path] 目录路径。[br]
	/// [param p_extensions] （可选）参与计算的文件扩展名列表，如 ["png", "jpg"]。空列表表示所有文件，大小写不敏感。[br]
	/// [return] MD5 哈希字符串，若目录无法打开则返回空字符串。
	static String compute_dir_hash(const String &p_path, const PackedStringArray &p_extensions = {});
	/// 递归计算目录及所有子目录的哈希。[br][br]
	/// [param p_path] 起始目录路径。[br]
	/// [param p_extensions] （可选）参与计算的文件扩展名列表，如 ["png", "jpg"]。空列表表示所有文件，大小写不敏感。[br]
	/// [return] MD5 哈希字符串，若目录无法打开则返回空字符串。
	static String compute_dir_hash_recursive(const String &p_path,
			const PackedStringArray &p_extensions = {});
	/// 递归复制目录（目标目录会自动创建）。[br][br]
	/// [param p_src] 源目录路径[br]
	/// [param p_dst] 目标目录路径[br]
	/// [return] 错误码，OK 表示成功。
	static Error copy_recursive(const String &p_src, const String &p_dst);
	/// 递归删除目录（含所有子目录与文件）。[br][br]
	/// [param p_path] 要删除的目录路径[br]
	/// [return] 删除成功则返回 true。
	static bool delete_directory_recursive(const String &p_path);
	/// 获取目录树结构（字典形式）。[br][br]
	/// [param p_path] 要遍历的目录路径[br]
	/// [return] 目录信息字典，含键：name(String)、path(String)、directories(Array[Dictionary])、files(Array[String])；目录无效时返回空字典。
	static Dictionary get_directory_tree(const String &p_path);
	/// 通用的目录树遍历，支持文件/目录回调，回调返回 false 终止遍历。[br][br]
	/// [param p_path] 起始目录路径[br]
	/// [param p_file_handler] 处理文件的回调，签名为 func(file_path: String, file_name: String) -> bool，返回 false 终止遍历[br]
	/// [param p_dir_handler] 处理目录的回调（可选），签名为 func(dir_path: String, dir_name: String) -> bool，返回 false 终止遍历[br]
	/// [param p_recursive] 是否递归子目录（默认为 true）[br]
	/// [return] 是否完整执行完毕（未被回调终止），true 表示完整遍历。
	...
	/// 根据条件删除文件，返回实际删除数量。[br][br]
	/// [param p_root_path] 起始目录路径[br]
	/// [param p_should_delete] 判断是否删除的回调，签名为 func(file_path: String, file_name: String) -> bool[br]
	/// [return] 实际删除的文件数量。
